...
Runs callback after the specified delay time.
Returns `integer` which can be utilized by the [`clearTimeout()`](/miscellaneous/clear-timeout/) function.
...
- `integer` CancellationID for the timeout. Can be used to cancel the timeout with [`clearTimeout()`](/miscellaneous/clear-timeout/).
