...
<Card title="Ilja Jusupov (x4fab)" icon="pencil">
...
Assetto Corsa was originally built using C++ and Python. However, Python in AC is limited to basic UI and some telemetry data. CSP introduces **LuaJIT** for several key reasons:
...
<CardGrid>
  ...
  <Card title="Direct Memory Access" icon="setting">
  ...
  <Card title="Hot Reloading" icon="approve-check">
  ...
  <Card title="Safety" icon="warning">
    Lua provides a sandboxed environment, ensuring that scripts can be shared
...
<Aside type="note" title="About these Docs">
  ...
  functions and types found within the SDK. It aims to provide a clear reference for
  developers wanting to get their hands dirty with the API.
</Aside>
...
<Aside type="tip" title="Ready to start coding?">
  Head over to the [Getting Started](../getting-started/) guide to set up your
  environment and write your first script.
</Aside>
